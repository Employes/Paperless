...
| `amountSelectedTemplate` | `amount-selected-template` | The template for amount selected          | `(amount: number) => string` | `( 		amount: number 	) => 		formatTranslation( 			(amount === 1 				? this._locales.amount_selected 				: this._locales.amount_selected_plural 			)?.replace('{{amount}}', amount) 		)` |
